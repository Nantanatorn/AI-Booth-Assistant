...
//ใช้สำหรับ UnityรับเสียงและตอบกลับในUnity
  ...
  const sessionReady = ai.live
    .connect({
      ...
      config: {
        ...
        systemInstruction:
          "You are a helpful assistant. Respond in Thai. if you don't know the answer, just say 'ขอไปเช็คดูก่อนนะครับ'.",
        temperature: 0.4,
        tools: [
          {
  ...
  ws.on("message", async (raw) => {
    try {
      ...
      console.log("Sending ai_response", {
        ...
        byteLength: result.audio
          ? Buffer.from(result.audio, "base64").length
          : 0,
