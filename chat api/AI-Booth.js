...
// ✅ NEW: Error Handler Tool for "Cant" action
const ErrorHandlerTool = {
  name: "error_handler",
  description:
    "Handle errors when the AI cannot understand or process the user's request. Use action 'Cant' to respond with an error message.",
  parameters: {
    type: Type.OBJECT,
    properties: {
      action: {
        type: Type.STRING,
        description:
          'Action to perform. Use "Cant" when you cannot understand or process the request.',
        enum: ["Cant"],
      },
    },
    required: ["action"],
...
wss.on("connection", (ws, req) => {
          ...
          await handleFunctionCalls(session, message, (entry) => {
            if (ws.readyState === WebSocket.OPEN) {
              ...
              if (entry.name === "return_user_text" && entry.args?.text) {
                ...
                // ✅ Filter out system messages - don't display in user dialogue
                if (userText.startsWith("[SYSTEM_ERROR]")) {
                  console.log(`🔇 [return_user_text] Skipping system message`);
                  return; // Don't echo or save system messages
                }

                console.log(`🎤 [return_user_text] Echoing: "${userText}"`);
                ws.send(
                  JSON.stringify({
  ...
  ws.on("message", async (raw) => {
    try {
      ...
      // ✅ 3.5 Interrupt (Cancel Generation)
      else if (payload.type === "interrupt") {
        console.log("🛑 Interrupt Signal Received");
        // Clear any accumulated text being buffered
        currentModelText = "";
        // Optional: Force send activityStart to ensure generation stops (redundant if client sends it, but safe)
        // const liveSession = session || (await sessionReady);
        // if (liveSession) liveSession.sendRealtimeInput({ activityStart: {} });
      }
      ...
      // ✅ 5. Action Handler (Unity sends action commands)
      else if (payload.type === "action" && payload.action === "Cant") {
        console.log("🚫 Action: Cant received from Unity");

        const liveSession = session || (await sessionReady);
        if (liveSession) {
          // Send instruction to AI to speak error message (turnComplete triggers response)
          liveSession.sendClientContent({
            turns: [
              {
                role: "user",
                parts: [
                  {
                    text: "[SYSTEM_ERROR] พูดตอบกลับผู้ใช้ว่า: เกิดข้อผิดพลาด พูดใหม่อีกครั้งนะคะ",
                  },
                ],
              },
            ],
            turnComplete: true, // This triggers AI to respond with audio
          });
        }
      }
